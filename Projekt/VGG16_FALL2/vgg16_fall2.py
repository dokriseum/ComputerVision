...
train_datagen = ImageDataGenerator(
    ...
    rotation_range=45,                 # Erhöht die Spanne der zufälligen Rotation
    width_shift_range=0.2,             # Erhöht die Spanne für horizontale Verschiebungen
    height_shift_range=0.2,            # Erhöht die Spanne für vertikale Verschiebungen
    shear_range=0.25,                  # Erhöht die Scherintensität
    zoom_range=[0.8, 1.2],             # Ändert den Bereich für zufälliges Zoomen
    channel_shift_range=50.0,          # Intensität der Farbkanalverschiebung
    horizontal_flip=True,              # Horizontales Flippen zulassen
    vertical_flip=True,                # Vertikales Flippen zulassen
    brightness_range=[0.5, 1.5],       # Erweitert den Bereich für zufällige Helligkeitsänderungen
    fill_mode='reflect',               # Ändert die Füllmethode für neu erschaffene Pixel
...
plt.ylabel('Loss')
plt.legend()
plt.title('F2 Training and Validation Loss')
...
plt.ylabel('Accuracy')
plt.legend()
plt.title('F2 Training and Validation Accuracy')
